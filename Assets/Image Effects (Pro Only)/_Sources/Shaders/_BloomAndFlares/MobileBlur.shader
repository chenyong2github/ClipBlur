...
Shader "Hidden/FastBlur" {
	...
	CGINCLUDE
		...
		{	
			...
			//circle			
			/*
			half2 center = half2(0.36, 0.75);
			half2 v = i.uv - center;
			v.y *= _ScreenParams.y / _ScreenParams.x;
			color *= 1 - step(length(v), 0.1);
			*/
	...
	Pass {		
		...
		Pass{
			...
			{
				fixed4 color = tex2D(_SourceTex, i.uv);
				fixed4 c = tex2D(_MainTex, i.uv);
				return max(0, (1 - c.a)) * color + c*(1-_Darken);
			}
			ENDCG
		}
	}
